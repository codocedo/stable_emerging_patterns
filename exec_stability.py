...
import sys
from libs.stability import stability
...
def execute_cbo_stability(filepath, **exec_params):
    """
    ...
    stab_embed = lambda extent, pclass, theta: stability(theta,
                                                         len(extent.intersection(pclass)),
                                                         len(extent) -
                                                         len(extent.intersection(pclass)))
    grate = lambda extent, pclass: float(len(extent.intersection(pclass)))/len(extent)


    cbo_params = {
        'min_sup':exec_params['min_sup'],
        'print':lambda extent, intent, depth:
                print('\t'*depth, '->',
                      extent,
                      intent,
                      'grate:{}'.format(grate(extent, exec_params['pclass'])),
                      'stab:{}'.format(stab_embed(extent, exec_params['pclass'], exec_params['theta'])),
                      '- EP' if grate(extent, exec_params['pclass']) >= exec_params['theta'] else ''
                     )
    }
...
if __name__ == "__main__":
    exec_params = {
        'min_sup':0.2,
        'pclass':set([1, 3, 4, 7]), # POSITIVE CLASS
        'theta':0.6
    }
    execute_cbo_stability(sys.argv[1], **exec_params)
